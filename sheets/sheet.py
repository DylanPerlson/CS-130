...
class Sheet:
    ...
    def set_cell_contents(self, workbook_instance, location, contents):
        ...
        for parent_cell in\
            ...
            if parent_r > MAX_ROW or parent_c > MAX_COL:
                self.cells[(row,col)].evaluated_value = Cell(
                    CellError(CellErrorType.BAD_REFERENCE, 'Out of bounds reference'))
    ...
    def retrieve_cell_references(self,workbook_instance,location,row,col):
        """Helper function that returns the references in a cell's formula."""
        if self.cells[(row,col)].parse_necessary is False:
